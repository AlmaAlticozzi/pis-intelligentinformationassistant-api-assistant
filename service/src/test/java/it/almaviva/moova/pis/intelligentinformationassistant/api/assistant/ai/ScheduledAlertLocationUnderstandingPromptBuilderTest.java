...
class ScheduledAlertLocationUnderstandingPromptBuilderTest {
    ...
    void promptDistinguishesMonitoredStopPointsFromFiltersAndPlatforms() {
        ...
        assertThat(request.userPrompt())
                ...
                .contains("Journey/service/train cancellation or suppression words are not locations")
                .contains("\"suppressed journeys at X\" means X is MONITORED_STOP_POINT")
                .contains("\"arrival suppressed journeys at X\" means X is MONITORED_STOP_POINT")
                .contains("quante corse ... ci sono a X")
                .contains("X is MONITORED_STOP_POINT with departure orientation")
                .contains("con destinazione X")
                .contains("FILTER_DESTINATION_STOP_POINT when a monitored location is already clear")
                .contains("che passano da X")
                .contains("FILTER_ROUTE_STOP_POINT, not monitoring scope")
                .contains("\"journeys at X with cancelled stop Y\" means X is MONITORED_STOP_POINT and Y is")
