...
public class ScheduledServiceDataRuntimeDataSourceBindingResolver implements RuntimeDataSourceBindingResolver {

    private final AgentRuntimePackageConfiguration configuration;

    public ScheduledServiceDataRuntimeDataSourceBindingResolver(AgentRuntimePackageConfiguration configuration) {
    ...
    public boolean supports(AgentActivationSnapshot snapshot) {
        return snapshot != null
                && "SCHEDULED_INTERPRETER".equals(snapshot.interpreterType())
                && "SCHEDULE".equals(snapshot.triggerType())
                && "ServiceDataStopPointJourneysV2".equals(snapshot.inputModel())
    ...
    public List<AgentRuntimeSubmission.AgentRuntimeDataSourceBinding> resolve(AgentActivationSnapshot snapshot) {
        AgentRuntimePackageConfiguration.ConnectorConfiguration connector = configuration.scheduledServiceDataConnector();
        if (connector == null || connector.operationRef() == null || connector.operationRef().isBlank()) {
