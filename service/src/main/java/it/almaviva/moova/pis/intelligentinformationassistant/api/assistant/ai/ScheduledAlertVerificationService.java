...
public class ScheduledAlertVerificationService {
    ...
    @Inject
    ScheduledSnapshotEvaluationCanonicalizer snapshotEvaluationCanonicalizer;
    ...
    public AlertVerificationOutcome verify(
        ...
        if (journeyCancellationHints.hasGenericJourneyCancellation()) {
            System.out.println("[IIA][ALERT_SCHEDULED_VERIFY][CANCELLATION_SEMANTIC] intent=GENERIC_JOURNEY_CANCELLATION direction=UNSPECIFIED exclusive=false");
            ...
            System.out.println("[IIA][ALERT_SCHEDULED_VERIFY][CANCELLATION_SEMANTIC] intent="
                    ...
                    + " exclusive=" + isExclusiveCancellation(constraint));
        }
    ...
    private AlertVerificationOutcome canonicalizeJourneyCancellationOutcome(
        ...
        ScheduledSnapshotEvaluationCanonicalizer canonicalizer = snapshotEvaluationCanonicalizer == null
                ? new ScheduledSnapshotEvaluationCanonicalizer()
                : snapshotEvaluationCanonicalizer;
        return canonicalizer.normalizeScheduledTechnicalSpecificationAndBlueprint(outcome, hints).outcome();
    }

    private boolean isExclusiveCancellation(ScheduledAlertJourneyCancellationConstraint constraint) {
        return constraint != null
                && (constraint.cancellationIntent() == ScheduledAlertJourneyCancellationConstraint.CancellationIntent.ARRIVAL_ONLY_JOURNEY_CANCELLATION
